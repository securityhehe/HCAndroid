...
) : Serializable, IListData {

    override fun getMaxAmount(): String {
        return maxAmount.toString()
    }

    override fun getLoanDuration(context: Context): String {
        return String.format(context.getString(R.string.mall_loan_duration), borrowDays)
    }

    override fun getDayInterest(context: Context): String {
        return String.format(context.getString(R.string.mall_daily_interest),doubleFormatNoSymbo(dayRate))
    }

    override fun getCanBuyFlag(): Boolean {
        return isCanBuy
    }

    override fun getChangeMoney(): Boolean {
            return true
    }

    override fun getShowPlatformName(): Boolean {
        return false

    }
    override fun _getPlatformName(): String {
        return platformName
    }
}
...
open class DiversionProduct(
    ...
    override fun getMaxAmount(): String {
        return borrowLimit.toString()
    }

    override fun getLoanDuration(context: Context): String {
       return String.format(context.getString(R.string.mall_loan_duration), timeLimit)
    }
...
interface IListData {
    ...
    fun getLoanDuration(context: Context): String
    fun getDayInterest(context: Context): String
    fun getCanBuyFlag(): Boolean
    fun getChangeMoney(): Boolean
    fun getShowPlatformName(): Boolean
    fun _getPlatformName():String
}

/**
 * 数值格式化 - 12345.00
 */
fun doubleFormatNoSymbo(args: Any?): String? {
    return if (args != null && !TextUtils.isEmpty(args.toString())) {
        val number = args.toString()
        try {
            DecimalFormat("#0.00").format(getDouble(number))
        } catch (e: Exception) {
            e.printStackTrace()
            number
        }
 ...
 */
fun getDouble(args: String?): Double {
    return try {
        if (TextUtils.isEmpty(args)) {
            0.0
        } else {
            BigDecimal(args).toDouble()
        }
